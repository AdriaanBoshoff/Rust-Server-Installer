...
uses
  ...
  ActiveX, IdSSLOpenSSL, IdIOHandler, IdIOHandlerSocket, IdIOHandlerStack, IdSSL;
...
begin
  ...
  try
    ...
    begin
      ...
      ExtractZip(frmssldownloader.path, frmssldownloader.extractpath);
    end
    else
    begin
      frmssldownloader.Caption := 'Finalizing...';
...
var
  total, currentsize, maxsize: Real;
begin
  ...
  begin
    ...
    total := Round(total);
    currentsize := progressbarstatus / 1048576;
    maxsize := maxprogressbar / 1048576;
    frmssldownloader.lblprogress.Caption := FloatToStr(total) + ' %';
    frmssldownloader.Caption := 'Downloading ' + FloatToStr(Round(currentsize)) + ' MB of ' + FloatToStr(Round(maxsize)) + ' MB';
  end;
end;
